...
estControl.postEstudiante = (req, res) => {
    ...
    const estudianteEncontrado = db.estudiantes.find((est) => est.id == id)
    if (estudianteEncontrado) {
        res.status(400).send("Estudiante ya existe");
        return;
    }
...
estControl.putEstudiante = (req, res) => {
    ...
    const estudianteEncontrado = db.estudiantes.find((est) => est.id == req.params.id)
    if (!estudianteEncontrado) {
        res.status(400).send("Estudiante no existe");
        return;
    }
    const estudiante = db.estudiantes.find(
        (est) => est.id == req.params.id
